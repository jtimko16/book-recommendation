...
def fetch_genre(isbn: str) -> dict:
    ...
    try:
        ...
        if response.status_code == 429:  # Too many requests
            retry_after = int(response.headers.get("Retry-After", 1))  # Get retry time in seconds
            print(f"Rate limit exceeded for ISBN {isbn}. Retrying after {retry_after} seconds...")
...
def fetch_all_genres(isbn_list: list[str], max_workers=5, delay=1) -> pd.DataFrame:
    results = []
